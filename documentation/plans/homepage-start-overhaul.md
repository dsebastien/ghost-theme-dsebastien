...
> **4.5b — articles #12–30 content pass** → moved to a vault TaskNote (full ranked table + exec notes there): `20 Actions/24 Tasks/2026/07/Articles 12-30 GEO content pass (answer-first intros + FAQs) (Task).md`, linked to **[[dSebastien Website (Project)]]** (Medium, Backlog). Same change-set + surgical-patch mechanism as the shipped top-11.

> **Article `FAQPage` schema** (was 4.7) → moved to a vault TaskNote: `20 Actions/24 Tasks/2026/07/Add per-article FAQPage JSON-LD to Ghost articles (Task).md`, linked to **[[dSebastien Website (Project)]]** (Low priority, Backlog). Decision captured there: no theme-global `post.hbs` emitter (invalid on FAQ-less posts + Handlebars can't extract the Q&A); do static per-article `codeinjection_head` injection if cheap; genuinely marginal.
...
## Measurement
Plausible goals live. Targets (first 30d post-change vs pre-baseline): `/`→`/start/` CTR ≥8% of unique home visitors; email signup rate on `/`+`/start/` +50%; fewer `/start/`→store clicks but higher store-page conversion (watch store analytics, not just clicks); checklist lead-magnet >0 downloads; start.hbs TTFB delta <100ms with ~10-12 `{{#get}}`. **GEO scoreboard → vault TaskNote:** `20 Actions/24 Tasks/2026/07/Set up GEO measurement - Plausible AI-referrer segment + citation spot-checks (Task).md` ([[dSebastien Website (Project)]], Medium, Backlog). Bot access confirmed clear (2026-07-21 — robots.txt + Cloudflare); the task covers the Plausible AI-referrer segment (`chatgpt.com`/`perplexity.ai`/`gemini.google.com`) + periodic citation spot-checks. No rich-result to measure against — citation presence is the metric.

## The One Thing
**All of Tier 4 (article-level GEO) is shipped or spun out to vault TaskNotes** — top-11 pass live, TOC/freshness/schema in place, AI-series chain wired; the remaining GEO work (4.5b #12–30 content pass, per-article `FAQPage`, GEO measurement) now lives as TaskNotes under [[dSebastien Website (Project)]]. **What's left in THIS plan is the unshipped homepage/`/start/` conversion work in Tiers 2–3: 2.4 (extract `layer-stack.hbs`), 3.1 (testimonial wall), 3.3 (meta description + OG), 3.4 (sticky-CTA on home + start), 3.5 (cleanups).** The single highest-leverage of those is **3.4 (sticky-CTA)** — smallest change, both long-scroll pages already have the `@custom` copy, and it directly targets conversion on the two pages people actually read end-to-end.
