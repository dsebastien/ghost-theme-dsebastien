...
## The One Thing
Run **4.6's prerequisite check now** — verify `GPTBot`/`PerplexityBot`/`ClaudeBot`/`ChatGPT-User`/`Google-Extended` aren't blocked in Cloudflare + robots.txt. 3.2 + 4.1 + 4.2 just made home, `/start/`, and all ~800 articles maximally citable, but if the answer-bots can't crawl, none of that signal is ever read — this one check gates the ROI of everything shipped this week. After that, **4.3 (auto TOC)** is the next theme-global lever (citable anchors + lower bounce), then the 4.5 content pass on the top ~30 articles.
