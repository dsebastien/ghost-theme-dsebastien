...
### 4.3 — TOC anchors: slug IDs, not `heading-N` (theme-global) — *narrowed*
The TOC UI is already shipped (`main.js` L162). But headings without a Ghost-assigned id get **index-based** IDs (`heading-0`, `heading-1`… — L182-186), so most articles' anchors are `#heading-3`: **not citable** (meaningless to an answer engine) and **not stable** (insert a section → every downstream anchor shifts, breaking external deep-links). This defeats the one GEO benefit a TOC is supposed to give. Fix (~5 lines): slugify `h.textContent` into the id with collision dedup, so anchors become `#how-to-build-a-pkm-system`. H2-only is fine (skip H3s — keeps the TOC clean).
...
## The One Thing
Make **4.3's TOC anchors real slugs** — a ~5-line `main.js` change swapping the index-based `heading-N` IDs for slugified `#how-to-build-a-pkm-system` anchors (with collision dedup). The TOC UI already ships and 3.2/4.1/4.2 are live, but the anchors are currently `#heading-3` — meaningless to answer engines and unstable, so the "citable, deep-linkable sections" GEO payoff isn't actually being delivered. Cheapest remaining theme-global GEO win; after it, the frontier is the **4.5** answer-first content pass on the top ~30 articles.
