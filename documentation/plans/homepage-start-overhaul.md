...
## The One Thing
Ship **2.5 (answer-first blocks + semantic markup)** — 2.6 just pointed internal PageRank at `/start/` and its five layer pages; the compounding follow-up is making those now-well-linked pages *extractable and rankable*: a 100–150-word definition block after home's H2, a "What is knowledge mastery?" block on `/start/`, and a real `<h2>` + semantic `<ol>` on the Five Layers. Pure copy/markup, lowest risk, and it turns the link equity into actual search + GEO visibility.
