...
### 4.7 — Article `FAQPage` schema emitter (theme-global)
Every article in 4.5/4.5b now has a visible `## Frequently Asked Questions` block, but none emit `FAQPage` JSON-LD (the theme emits `FAQPage` on home only, via `json-ld-extras.hbs`). Add a marker-keyed emitter to `post.hbs`: detect the FAQ section (e.g. by the `Frequently Asked Questions` heading or a wrapping marker) and emit a `FAQPage` node whose Q&As mirror the visible DOM. One `post.hbs` deploy retro-fits schema to every article that has the block. (FAQ earns no Google rich result since 2023 — this is pure GEO/answer-engine signal, same rationale as the home `FAQPage`.) Note: Ghost renders the FAQ as plain h3+p, so the emitter likely needs a small JS pass or a Koenig HTML-card marker around the block — decide the detection method before building.
...
## The One Thing
**Measure, then widen.** The 4.5 top-11 content pass is shipped and live (every high-traffic article now leads with an answer-first block + FAQ, and all theme-global GEO levers 4.1–4.4 are in place). The next highest-leverage move is to set up the GEO scoreboard (Plausible AI-referrer segment for `chatgpt.com`/`perplexity.ai`/`gemini.google.com` + citation spot-checks on the questions these FAQs answer) so the impact is observable, then work the **4.5b** checklist (#12–30, worst-offenders first). Ship **4.7** (article `FAQPage` emitter in `post.hbs`) once there are enough FAQ blocks to retro-fit — one deploy schemas them all. **4.8** (vault series-chain backfill) is low-priority hygiene, do it anytime.
